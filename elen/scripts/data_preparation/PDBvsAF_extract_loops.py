...
from elen.shared_utils.utils_extraction import get_sequence_identity
from elen.shared_utils.utils_extraction import get_residues_around_loop
...
def get_total_residues_from_struct(struct):
    total_residues = 0
    for model in struct:
        for chain in model:
            for res in chain:
            ...
            for path_pdb in path_models: # iterate over list of .pdb files
                ...
                for model in chain_struct:
                    for chain_native in model:
                        ...
                        total_residues = get_total_residues_from_struct(fin_struct)
