...
from elen.data_preparation.utils_extraction import get_BioPython_DSSP, get_loop_positions
from elen.config import PATH_DSSP

def rosetta_numbering(chain):
    """Convert residue numbering of chain to Rosetta-style numbering (sequential, starting from 1)."""
    residue_idx = 1
    for res in chain:
        res.id = (" ", residue_idx, " ")
        residue_idx += 1
...
def main(args):
    ...
    for path_af3 in glob.glob(os.path.join(args.inpath_AF, "*.pdb")):
        print(f"Processing {path_af3}")
        
        structure = parser.get_structure('protein', path_af3)
        # get SS and loop positions
        ss, sequence = get_BioPython_DSSP(path_af3, PATH_DSSP)
        ...
        #print(f"ligand_residues: {ligand_residues}")
        sys.exit(0)
        if ligand_residues:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--nr_residues", type=int, default=30, help="Number of closest residues to include in the loop pocket.")
